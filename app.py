...
def download_models():
    ...
    model_files = {
        "cnn_custom_model.h5": "1IF__68Y3QtCwUpS3JdKbaZkCMR759h_z",
        "inceptionv3_model.h5": "1Yspoi5n07oUuiHfhbTRy_QkscG85BVbI",
        "inceptionresnet_model.h5": "1oiIWpbp4pjuMi06l1oV1INqYKUd-SX-g"
    }
...
cnn_input_shape = cnn_model.input_shape[1:3]
...
def predict():
    ...
    try:
        if filename.lower().endswith(("jpg", "jpeg", "png")):
            ...
            label, confidence = ensemble_predict(img_pil)
            return jsonify({'prediction': f"{label}"})

        elif filename.lower().endswith(("mp4", "avi", "mov")):
            predictions = process_video(filepath)
            pred_text = [f"Frame {i+1}: {label}" for i, (label, conf) in enumerate(predictions[:5])]
            return jsonify({'prediction': "<br>".join(pred_text)})
